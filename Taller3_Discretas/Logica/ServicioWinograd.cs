...
{
    class ServicioWinograd
    {
        
        private int[,] matrizC;
        
        public ServicioWinograd()
        {
           
        }
        public void multiplicarWinograd(int[,] matrizA, int[,] matrizB)
        {
            ...
            for (int i = 0; i < a.GetLength(0); i = i + 2)
            {
                for (int j = 0; j < a.GetLength(0); j = j + 2)
                {
                    for (int k = 0; k < a.GetLength(0); k = k + 2)
                    {
                        ...
                        int h0 = b[k + 1, j + 1];
                        //=======================================
                        int p1 = (c0+d0-a0)*(h0-f0+e0);
                        int p2 = a0*e0;
                        int p3 = b0*g0;
                        int p4 =(a0-c0)*(h0-f0);
                        int p5 = (c0+d0)*(f0-e0);
                        int p6 = (b0-c0+a0-d0)*h0;
                        int p7 = d0*(g0-h0+f0-e0);
                        //=======================================
                        matrizC[i, j] += p2 + p3;

                        //0,1
                        matrizC[i, j + 1] += p1 + p2 + p5 + p6;

                        // 1,0
                        matrizC[i + 1, j] += p1 + p2 + p4 + p7;

                        //1,1
                        matrizC[i + 1, j + 1] += p1 + p2 + p4 + p5;
